...
<body>
  <fieldset>
    <?php $pr = select_all('pr-details', ['username', $_SESSION['user'], 'pr-no', $_GET['pr']]); ?>
    ...
    <form name='edit-pr' method="post">
      <table>
        ...
        <tr><td>Jira Summary:</td><td><input type="text" name="jira-summary" value='<?php echo $pr[0]['jira-summary'] ?>' /></td></tr>
    ...
    <?php
      if(isset($_POST['jira-ticket'])) {
        ...
        update(['jira-ticket', 'jira-summary', 'pr-link', 'pr-no', 'pr-branch'], [$_POST['jira-ticket'], $_POST['jira-summary'], $_POST['pr-link'], $pr_number, $_POST['pr-branch']], 'pr-details', ['username', $_SESSION['user'], 'pr-no', $_GET['pr']]);
